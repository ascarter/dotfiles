...
This configuration requires Neovim 0.12 or newer. It manages editor plugins
and a small parser baseline, but it does not install language servers,
formatters, linters, language runtimes, or debug adapters.

Language servers and debug adapters are enabled only when their commands are
already available on `PATH`, normally from the current project's mise
environment.

Project-local augmentation uses Neovim's trusted `exrc` support. A project may
commit `.nvim.lua` and `.nvim/lsp/*.lua`; inspect `.nvim.lua` and run `:trust`
from that buffer before Neovim executes it.
