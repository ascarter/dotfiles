...
bios_info="$bios_vendor $bios_version ($bios_release_date)"
if [ -z "$bios_vendor" ] || [ -z "$bios_version" ] || [ -z "$bios_release_date" ]; then
    bios_info="Not Available"
fi
...
system_info="$system_manufacturer $system_product_name v$system_version (S/N: $system_serial_number)"
if [ -z "$system_manufacturer" ] || [ -z "$system_product_name" ] || [ -z "$system_version" ] || [ -z "$system_serial_number" ]; then
    system_info="Not Available"
fi
